...
const GRAPHIFY_METRICS = {json.dumps(graphify_metrics)}
...
function compressGraphify(project, question, output, sessionID) {{
  return new Promise((resolve) => {{
    const child = spawn(PYTHON, [GRAPHIFY_METRICS], {{ stdio: ["pipe", "ignore", "ignore"] }})
    child.on("error", () => resolve())
    const timer = setTimeout(() => {{ child.kill(); resolve() }}, 5000)
    child.on("close", () => {{ clearTimeout(timer); resolve() }})
    child.stdin.end(JSON.stringify({{ project, question, output, session_id: sessionID }}))
  }})
}}
...
export const TokenSaver = async () => ({{
  "tool.execute.before": async (input, output) => {{
    ...
    const match = typeof command === "string"
      ? command.match(/^graphify\\s+query\\s+(["'])([\\s\\S]*?)\\1(?:\\s|$)/)
      : null
  ...
  "tool.execute.after": async (input, output) => {{
    ...
    const graphifyMatch = command.match(
      /^graphify\\s+query\\s+(["'])([\\s\\S]*?)\\1(?:\\s|$)/,
    )
    const question = input.args?.__tokenSaverGraphifyQuestion || graphifyMatch?.[2]
    if (typeof question === "string") {{
      await compressGraphify(
        input.args?.workdir || process.cwd(), question, output.output, input.sessionID,
      )
    }}
